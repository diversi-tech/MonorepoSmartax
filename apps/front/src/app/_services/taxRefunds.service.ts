...
import { TAX_REFUNDS } from '../api-urls';
...
export class TaxRefundsService {
  ...
  createTaxRefunds(taxRefunds: TaxRefunds): Observable<TaxRefunds> {
    return this.http
      .post<TaxRefunds>(`${this.apiUrl}/create`, taxRefunds)
      .pipe(catchError(this.handleError<TaxRefunds>('createTaxRefunds')));
  }

  getAllTaxRefunds(): Observable<TaxRefunds[]> {
    return this.http
      .get<TaxRefunds[]>(`${this.apiUrl}/all`)
      .pipe(catchError(this.handleError<TaxRefunds[]>('getAllTaxRefunds', [])));
  }
  ...
  async updateTaxRefunds(
    id: string,
    taxRefunds: TaxRefunds
  ): Promise<TaxRefunds> {
    try {
      const response = await this.http
        .post<TaxRefunds>(`${this.apiUrl}/update/${id}`, taxRefunds)
        .toPromise();
      return response;
    } catch (error) {
  ...
  deleteTaxRefunds(id: string): Observable<boolean> {
    return this.http
      ...
      .pipe(catchError(this.handleError<boolean>('deleteTaxRefunds', false)));
  }
