...
export class PriorityService {
  ...
  private apiUrl = 'http://localhost:8080/Priority'; // Base URL for the Client API
  ...
  createPriority(Priority: Priority): Observable<Priority> {
    return this.http.post<Priority>(this.apiUrl, Priority, this.httpOptions)
      .pipe(
        catchError(this.handleError<Priority>('createClient'))
      );
  }
  ...
  getAllPrioritys(): Observable<Priority[]> {
    return this.http.get<Priority[]>(`${this.apiUrl}`)
      .pipe(
        catchError(this.handleError<Priority[]>('getAllClients', []))
      );
  }

  // Search for a Client by ID
  searchPriority(id: string): Observable<Priority[]> {
    return this.http.post<Priority[]>(`${this.apiUrl}/searchClient`, { id }, this.httpOptions)
      .pipe(
        catchError(this.handleError<Priority[]>('searchClient', []))
      );
  }

  // Update an existing Client
  updatePriority(client: Priority): Observable<Priority> {
    return this.http.put<Priority>(`${this.apiUrl}`, client, this.httpOptions)
      .pipe(
        catchError(this.handleError<Priority>('updateClient'))
      );
  }
  ...
  deletePriority(id: string): Observable<boolean> {
    return this.http.delete<boolean>(`${this.apiUrl}`, { ...this.httpOptions, body: { id } })
      .pipe(
        catchError(this.handleError<boolean>('deletePriority', false))
      );
  }
