...
export class CommunicationsController {
    ...
    async delete(@Body() body: { id: string }): Promise<void> {
        await this.communicationsService.deleteCommunication(body.id);
    }
    ...
    @UseGuards(AuthGuard, RoleGuard(3))
    @Post('DeleteOldComunication')
    @ApiOperation({ summary: 'Delete old communications and move them to the communicationArchives table' })
    async deleteOldComunications(): Promise<void> {
        await this.communicationsService.deletingOldCommunications();
    }
}
